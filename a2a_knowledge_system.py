...
async def demo_knowledge_system():
    ...
    logger.info("🧠 A2A Knowledge Management System Demo")
    logger.info("=" * 50)
    ...
    logger.info("✅ Created knowledge base and 3 agents")
    ...
    logger.info(f"Added {len(knowledge_base.knowledge_items)} knowledge items")
    ...
    logger.info(f"Agent 2 discovered {len(discovered)} relevant knowledge items:")
    for item in discovered:
        logger.info(f"  - {item.title} (confidence: {item.confidence:.2f})")
